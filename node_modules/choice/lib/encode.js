...
function textBefore(root, node) {
  ...
  while (node && node !== root) {
    ...
    while ((0, _utils.isElem)(node) && node.lastChild) {
      node = node.lastChild;
    }
...
function encodePosition(children, node, offset) {
  ...
  while (node) {
    if ((0, _utils.isText)(node) || node.nodeName === 'BR') {
      break;
  ...
  var textIndex = textBefore(child, node) + offset;

  return [childIndex, textIndex];
}

exports['default'] = encodePosition;
